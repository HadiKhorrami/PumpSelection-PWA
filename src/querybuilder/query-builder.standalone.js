...
<dl id="{{= it.group_id }}" class="rules-group-container" style="font-family: IranSans"> \
  <dt class="rules-group-header"> \
    <div class="btn-group pull-right group-actions"> \
      <button type="button" class="btn btn-xs btn-success" data-add="rule"> \
        <i class="{{= it.icons.add_rule }}"></i> افزودن قاعده \
      </button> \
      {{? it.settings.allow_groups===-1 || it.settings.allow_groups>=it.level }} \
        <button type="button" class="btn btn-xs btn-success" data-add="group"> \
          <i class="{{= it.icons.add_group }}"></i> افزودن گروه \
        </button> \
      {{?}} \
      {{? it.level>1 }} \
        <button type="button" class="btn btn-xs btn-danger" data-delete="group"> \
          <i class="{{= it.icons.remove_group }}"></i> حذف گروه \
        </button> \
      {{?}} \
    </div> \
  ...
  <div class="rule-header"> \
    <div class="btn-group pull-right rule-actions"> \
      <button type="button" class="btn btn-xs btn-danger" data-delete="rule"> \
        <i class="{{= it.icons.remove_rule }}"></i> حذف قاعده \
      </button> \
    </div> \
  </div> \
  {{? it.settings.display_errors }} \
...
QueryBuilder.defaults({
    ...
    sqlOperators: {
        ...
        EQUALS:     {op: 'EQUALS([$geom_field$], ?)'},
        DISJOINT:   {op: 'DISJOINT([$geom_field$], ?)'},
        INTERSECTS: {op: 'INTERSECTS([$geom_field$], ?)'},
        TOUCHES:    {op: 'TOUCHES([$geom_field$], ?)'},
        CROSSES:    {op: 'CROSSES([$geom_field$], ?)'},
        WITHIN:     {op: 'WITHIN([$geom_field$], ?)'},
        CONTAINS:   {op: 'CONTAINS([$geom_field$], ?)'},
        OVERLAPS:   {op: 'OVERLAPS([$geom_field$], ?)'},
        RELATE:     {op: 'RELATE([$geom_field$], ?)'},
        SETORGDWITHIN:    {op: '(SETORGDWITHIN([$geom_field$], ?, [$distance$]) = true)'},
        SETORGBEYOND:     {op: '(SETORGBEYOND([$geom_field$], ?, [$distance$]) = true)'}
    },
...
QueryBuilder.regional['en'] = {
  ...
  "operators": {
    ...
    "less_or_equal": "کمتر یا مساوی با",
      ...
      "INTERSECTS": "اشتراک داشته باشد با",
      "EQUALS": "برابر باشد با",
      "DISJOINT": "اشتراک نداشته باشد با",
      "TOUCHES": "تماس نداشته باشد با",
      "CROSSES": "عبور کند از",
      "WITHIN": "قرار گرفته باشد در داخل",
      "CONTAINS": "شامل باشد",
      "OVERLAPS": "هم‌پوشانی داشته باشد با",
      "RELATE": "مرتبط باشد با",
      "SETORGDWITHIN": "دارای حداکثر فاصله از...",
      "SETORGBEYOND": "دارای حداقل فاصله از...",
  },
  "errors": {
